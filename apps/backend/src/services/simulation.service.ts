import { graphService, Junction } from './graph.service';
...
export class SimulationService {
  ...
  public initializeState(lat: number, lon: number, initialSeverity: number): PropagationState {
    ...
    if (nearest) {
      ...
      console.log(`[Simulation] Initialized epicenter at ${nearest.id} (${nearest.name}) with severity ${initialSeverity}`);
    } else {
  ...
  public tick(state: PropagationState, interventions: Interventions): PropagationState {
      ...
      for (const edge of neighbors) {
        ...
        if (!newState.activeNodes[neighborId]) {
          ...
          if (roll < spreadChance) {
             ...
             console.log(`[Simulation] Tick ${newState.currentTick}: SPREAD ${nodeId} → ${neighborId} | roll=${roll.toFixed(3)} < chance=${spreadChance.toFixed(3)} | intensity=${childIntensity.toFixed(3)}`);
          } else {
             console.log(`[Simulation] Tick ${newState.currentTick}: NO SPREAD ${nodeId} → ${neighborId} | roll=${roll.toFixed(3)} >= chance=${spreadChance.toFixed(3)}`);
          }
        }
      }
    }
    ...
    for (const nodeId of Object.keys(newState.activeNodes)) {
      ...
      if (newState.activeNodes[nodeId].intensity < 0.05) {
        console.log(`[Simulation] Tick ${newState.currentTick}: RECOVERED ${nodeId} (intensity dropped below 0.05)`);
        delete newState.activeNodes[nodeId];
      } else {
        console.log(`[Simulation] Tick ${newState.currentTick}: DECAY ${nodeId} intensity=${newState.activeNodes[nodeId].intensity.toFixed(3)} (decay=${decayFactor})`);
      }
    }

    const activeCount = Object.keys(newState.activeNodes).length;
    console.log(`[Simulation] Tick ${newState.currentTick} complete — ${activeCount} active node(s): [${Object.keys(newState.activeNodes).join(', ')}]`);
    return newState;
  }
}

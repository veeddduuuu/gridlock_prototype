          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className="flex flex-col items-end gap-1">
                <span
                  className={`px-1.5 py-0.5 rounded-[4px] text-[9px] font-bold uppercase tracking-wider ${isDemobilized ? 'bg-green-500/10 text-green-500' : 'bg-orange/10 text-orange'}`}
                >
                  {b.rule_source.replace(/_/g, ' ')}
                </span>
                <span
                  className={`font-mono text-[10px] ${isDemobilized ? 'text-green-500 font-bold' : 'text-muted-foreground'}`}
                >
